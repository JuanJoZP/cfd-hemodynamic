...
El valor de $R_{\mathrm{arbol}}$ se puede aproximar de forma analítica aprovechando que conocemos el radio (semi-ancho) de los vasos en cada nivel gracias a la ley de Murray. Asumiendo el perfil de flujo de Poiseuille para canales en 2D (placas paralelas), la resistencia teórica de un conducto rectangular de anchura $w$ y altura $h$ es $R = \frac{12 \mu L}{h^3}$. Dado que la altura total del canal vaso es el diámetro ($h = 2r$), la resistencia local se simplifica:
\begin{equation}
  R = \frac{12 \mu L}{(2r)^3} = \frac{3 \mu L}{2 r^3}
\end{equation}

En la generación de esta malla, la longitud del vaso en cada nivel mantiene una proporción fijacon el radio ($\alpha$), de manera que $L = \alpha r$. Al sustituir esta definición, se observa que la resistencia local de la sección de vaso en el primer nivel del árbol (de radio $r_0$) es:
\begin{equation}
  R_0 = \frac{3 \mu \alpha}{2 r_0^2}
\end{equation}
...
\begin{equation}
  R_{\mathrm{vaso}, j} = \frac{3 \mu \alpha}{2 r_j^2} = \frac{3 \mu \alpha}{2 r_0^2} \left(\frac{1}{2^{-2j/3}}\right) = R_0 \, 2^{2j/3}
\end{equation}
...
\begin{equation}
  R_{\mathrm{nivel}, j} = \frac{R_{\mathrm{vaso}, j}}{2^j} = R_0 \frac{2^{2j/3}}{2^j} = R_0 \, 2^{-j/3}
\end{equation}

La resistencia total del árbol se obtiene sumando las resistencias de cada nivel:
\begin{equation}
  R_{\mathrm{arbol}} = \sum_{j=0}^{N_{\mathrm{gen}}-1} R_{\mathrm{nivel}, j} = \frac{3 \mu \alpha}{2 r_0^2} \sum_{j=0}^{N_{\mathrm{gen}}-1} 2^{-j/3}
\end{equation}
...
Para investigar de qué manera la morfología de la lesión afecta las variables hemodinámicas y los índices diagnósticos, se evaluó el impacto de la forma geométrica de la oclusión. En particular, se buscó determinar si existe un comportamiento diferencial entre una estenosis prolongada y una focal, si la forma abrupta de la lesión supone un factor crítico, o si el impacto sobre el flujo vascular está regido principalmente por la relación entre la pendiente de transición y el nivel de severidad.

Con este propósito, se plantearon $9$ escenarios de simulación correspondientes a todas las combinaciones posibles de tres niveles de severidad ($\eta \in \{0.3, 0.6, 0.9\}$) y tres parámetros de pendiente ($\alpha \in \{0.2, 0.5, 0.8\}$).

% Espacio reservado para visualizar la diferencia de morfología
\begin{figure}[htbp]
  \centering
  % \includegraphics[width=0.48\textwidth]{figures/estenosis_focal.png}
  % \hfill
  % \includegraphics[width=0.48\textwidth]{figures/estenosis_prolongada.png}
  \caption{Comparación geométrica de perfiles: estenosis focal/abrupta (izquierda) frente a estenosis prolongada/gradual (derecha).}
...
- explicacion eleccion de intervalo de resistencia
...
\subsection{Impacto de la forma de la estenosis}

el experimento con severidad y pendiente a ffr/cfr
