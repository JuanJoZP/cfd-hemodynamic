...
Donde $u: \mathbb{R}^d \times \mathbb{R}_+ \rightarrow \mathbb{R}^d$, $p: \mathbb{R}^d \times \mathbb{R}_+ \rightarrow \mathbb{R}$ corresponden a velocidad y presión, respectivamente, como funciones de espacio y tiempo, $d \in \{2,3\}$ es la dimensión espacial, $\rho = 1055 \, \text{kg}/\text{m}^3$ es la densidad de la sangre, $\mu = 0.0035\, \text{Pa} \cdot \text{s}$ es la viscosidad dinámica y $f: \mathbb{R}^d \rightarrow \mathbb{R}^d$ es una fuerza externa aplicada al sistema.
...
\subsection{Impacto de la ubicación axial de la estenosis}
...
% no estoy seguro de la otra idea, depronto sí pero no se miro que fuera que la resistencia interna de la estenosis opacara la de la condición de frontera, otra cosa importante ahí fue que la resistencia se acopló de forma explicita y no se sabe eso como afecte. 
% la ultima idea hay que revisarla y si es cierta revisar porque podria ser que la resistencia interna esta varios ordenes de magnitud arriba
Una posible explicación para la discrepancia entre nuestra observación y la sensibilidad reportada en la literatura es la dominancia de la impedancia geométrica en nuestro modelo. La resistencia hidráulica de la estenosis se vuelve predominante al aumentar la severidad, de modo que las fluctuaciones de la resistencia microvascular quedan enmascaradas. Una hipótesis explicativa es que, dado que la resistencia de la estenosis varía en varios órdenes de magnitud debido a la relación no lineal con la reducción del radio, la variación en la resistencia microvascular simulada resulta insuficiente para alterar significativamente la diferencia de presiones. Alternativamente, esta discrepancia podría atribuirse a las simplificaciones del modelo, que podrían ocultar efectos presentes en configuraciones más completas.

En primer lugar, la suposición de paredes rígidas en la arteria y el árbol es una limitación. El trabajo de Konala et al. \cite{Konala2011Influence} muestra que omitir el movimiento de las paredes vasculares sobreestima la caída de presión entre un $27.7\%$ y un $37.6\%$ en estenosis severas. La falta de interacción fluido-estructura también distorsiona la distribución del estrés de corte en la pared, concentrándolo de forma artificial sobre la curvatura externa de los vasos \cite{Failer2021Impact, Kandangwa2022Influence}.

Por otro lado, el tratamiento de la sangre como un fluido Newtoniano subestima la caída de presión total entre un $7.8\%$ y un $12.4\%$ según Dutra et al. \cite{Dutra2021Effect}. Johnston \cite{Johnston2006Modelling} y Wajihah et al. \cite{Wajihah2023Review} reportan que el modelo no-Newtoniano adecuado captura variaciones en el estrés de corte de pared en regiones de bajo flujo y recirculación post-estenótica, donde la viscosidad aumenta de forma no lineal debido a la agregación eritrocitaria.
