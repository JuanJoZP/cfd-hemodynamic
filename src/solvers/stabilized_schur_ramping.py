...
class Solver(SolverBase):
    ...
    def solveStep(self):
        ...
        # Ramping Logic
        # We need to track time or steps. SolverBase doesn't have step count, but we can infer or add it.
        # Let's add a step counter attribute if not present
        ...
        if self.step_count <= ramp_steps:
            alpha = self.step_count / ramp_steps
            print(f"[INFO] Ramping BCs: step {self.step_count}, alpha={alpha}")

            # Scale BCs
            # We need to scale the underlying functions of the DirichletBCs in self.bcu_d
            # Note: bcu_d are dolfinx.fem.DirichletBC objects.
            # We access the function via bc.function_space (no directly) or bc.g (the value)
            # Dolfinx 0.8+: bc.g is the function.
            # We need to be careful: if multiple BCs share the same function, we might double scale.
            # Also we must restore them after solve.

            original_values = []

            for bc in self.bcu_d:
                # Check if bc has a function g
                if hasattr(bc, "g") and isinstance(bc.g, Function):
                    # Store copy of original array
                    original_vals = bc.g.x.array.copy()
                    original_values.append((bc.g, original_vals))
            ...
            try:
                ...
                for func, vals in original_values:
                    func.x.array[:] = vals
