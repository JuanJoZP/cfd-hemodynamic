...
class Solver(SolverBase):
    ...
    def __init__(
        ...
        # We will handle Stokes initialization in setup() where BCs are available
    ...
    def setup(self, bcu: list[BoundaryCondition], bcp: list[BoundaryCondition]) -> None:
        ...
        # Check if u_prev is close to zero (meaning no initial velocity was set)
        # We can also check a flag, but for now let's check norm
        if self.u_prev.x.petsc_vec.norm() < 1e-9:
            print(
                "[INFO] No initial velocity provided. Solving Stokes problem for initial guess..."
            )
            try:
                stokes_solver = StokesSolver(self.mesh, self.rho, self.mu, self.f)

                # Concat all BCs for stokes
                bcs_stokes = [bc.getBC(stokes_solver.V) for bc in bcu] + [
                    bc.getBC(stokes_solver.Q) for bc in bcp
                ]

                stokes_solver.solve(bcs=bcs_stokes)

                # Interpolate result into our current solution function
                self.u_prev.interpolate(stokes_solver.u_sol)
                self.p_prev.interpolate(stokes_solver.p_sol)

                # Also update current solution vars
                self.u_sol.interpolate(stokes_solver.u_sol)
                self.p_sol.interpolate(stokes_solver.p_sol)

                print("[INFO] Stokes initialization complete.")
            except Exception as e:
                print(
                    f"[WARN] Stokes initialization failed: {e}. Exploring zero initial guess."
                )
