...
# and directional do-nothing (DDN) condition at outlet.
# DDN: σ(u,p)n = ½(u·n)₋ u  — stabilization only activates during backflow.
# Ref: Braack & Mucha (2014), also discussed in Simon & Notsu (2021).
...
class Solver(SolverBase):
    ...
    def __init__(
        ...
        if v_max is None:
            raise ValueError(
                "v_max is required for stabilized_schur_vascularbc_ddn. "
    ...
    def setup(
        ...
        # 2. Directional do-nothing (DDN) at outlet:
        #    σ(u,p)n = ½(u·n)₋ u   (Braack & Mucha, 2014)
        #    Replaces the full stress σn on Γ_out.  During outflow (u·n ≥ 0)
        #    this reduces to the standard do-nothing σn = 0.  During backflow
        #    (u·n < 0) the term ½ρ(u·n)₋(u·v) penalises reverse flow.
        #    Semi-implicit: the coefficient uses u_prev, the velocity uses u_mid.
        n = FacetNormal(self.mesh)
        v = self._v_test
        u = self.u_mid
        un_prev = dot(self.u_prev, n)
        un_minus = conditional(le(un_prev, 0.0), un_prev, 0.0)
        self.F -= 0.5 * un_minus * dot(u, v) * ds_out
