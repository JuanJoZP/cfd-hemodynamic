...
# and resistance outlet BC (P = R * Q) updated via fixed-point iteration between timesteps.
...
class Solver(SolverBase):
    ...
    def __init__(
        ...
        if p_inlet is None:
            raise ValueError(
                "p_inlet is required for stabilized_schur_vascularbc. "
                "Pass it via CLI: --p_inlet <value>"
            )
        if R_resistance is None:
            raise ValueError(
                "R_resistance is required for stabilized_schur_vascularbc. "
    ...
    def setup(
        ...
        self.F += self._p_out_const * dot(v, n) * self._ds_out

        # 3. Flux form for resistance BC: Q = ∫_outlet u·n dΓ
        self._Q_form = form(dot(self.u_sol, n) * self._ds_out)

        # 4. Nitsche for tangential velocity (u_T = 0) on inlet and outlet
        u = self.u_mid
        h = self._h
        u_T = u - dot(u, n) * n
        v_T = v - dot(v, n) * n
        for ds_bc in (ds_in, self._ds_out):
            self.F += (
    ...
    def _update_outlet_pressure(self) -> None:
        ...
        PETSc.Sys.Print(f"  Resistance BC: Q={Q:.6e}, p_outlet={p_outlet_phys:.4f}")
