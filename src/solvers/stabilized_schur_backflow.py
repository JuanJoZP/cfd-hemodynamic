...
# Outlet: do-nothing (zero traction) + backflow stabilization (Moghadam et al. 2011, Eq. 10).
#
# The backflow stabilization adds:
#   -beta * rho * (u·n)_- * (u · v) ds_out
# where (u·n)_- = (u·n - |u·n|) / 2  (active only when flow reverses at the outlet).
...
class Solver(SolverBase):
    ...
    def __init__(
        ...
        if v_max is None:
            raise ValueError(
                "v_max is required for stabilized_schur_backflow. "
        ...
        # do-nothing: the boundary integral from IBP is NOT imposed (natural zero-traction)
    ...
    def setup(
        ...
        # 1. Outlet subdomain measure for backflow stabilization
        ds_out = Measure(
            "ds",
            domain=self.mesh,
        ...
        # 2. Backflow stabilization (Moghadam et al. 2011, Eq. 10):
        ...
        # 4. Dirichlet BCs (wall no-slip + inlet parabolic from scenario)
