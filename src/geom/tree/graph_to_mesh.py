...
def parse_gxl(gxl_path, voxel_width=0.04):
    vw = voxel_width
...
def build_mesh(
    ...
    root = next(filter(lambda n: " root node " in node_types[n], nodes.keys()))
    ...
    while len(queue) > 0:
        ...
        if " terminal node " in node_types[current_node]:
            continue
...
def tag_and_mesh_with_gmsh(
    brep_path: str, nodes: dict, node_types: dict, tol: float, out_msh: str
):
    """
    ...
    # inlet: root node
    root_id = next(filter(lambda n: " root node " in node_types[n], nodes.keys()))
    ...
    if root_id in assigned:
        ...
        gmsh.model.addPhysicalGroup(2, [assigned[root_id]], 1)
    ...
    if outlet_surfaces:
        ...
        gmsh.model.addPhysicalGroup(2, outlet_surfaces, 2)
        gmsh.model.setPhysicalName(2, 2, "outlets")
    ...
    if wall_surfaces:
        ...
        gmsh.model.addPhysicalGroup(2, wall_surfaces, 3)
